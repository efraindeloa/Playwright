...
test('Crear 29 servicios', async ({ page }) => {
  ...
  test.setTimeout(1800000); // 30 minutos - tiempo suficiente para crear 29 servicios
  
  // --- ADMINISTRAR SERVICIOS (solo una vez al inicio) ---
  await showStepMessage(page, '🔧 NAVEGANDO A ADMINISTRAR SERVICIOS');
  await page.waitForTimeout(1000);
  ...
  const totalServicios = 29;

  for (let i = 0; i < totalServicios; i++) {
    const servicioNum = i + 1;
    console.log(`\n📦 CREANDO SERVICIO ${servicioNum}/${totalServicios}`);
    
    try {
      // Verificar que estamos en la página correcta antes de crear
      const crearServicioVisible = await page.getByText('Crear servicio').isVisible({ timeout: 5000 }).catch(() => false);
      if (!crearServicioVisible) {
        // Si no estamos en la página correcta, navegar
        await page.goto(PROVIDER_SERVICES_URL);
        await page.waitForTimeout(2000);
      }

      // --- CREAR SERVICIO ---
      await showStepMessage(page, `➕ CREANDO NUEVO SERVICIO (${servicioNum}/${totalServicios})`);
      await page.waitForTimeout(1000);
      ...
      // --- SELECCIONAR CATEGORÍA ALEATORIA ---
      const nombreCategoria = await selectRandomCategory(page, `SELECCIONANDO CATEGORÍA ALEATORIA (${servicioNum}/${totalServicios})`);
      if (!nombreCategoria) {
        throw new Error('❌ No se pudo obtener el nombre de la categoría');
      }
      await page.waitForTimeout(2000);
      ...
      const nombreSubcategoria = await selectRandomCategory(page, `SELECCIONANDO SUBCATEGORÍA ALEATORIA (${servicioNum}/${totalServicios})`);
      if (!nombreSubcategoria) {
        throw new Error('❌ No se pudo obtener el nombre de la subcategoría');
      }
      await page.waitForTimeout(2000);
      ...
      if (necesitaSubcategoriaAnidada) {
        console.log(`🔍 Subcategoría "${nombreSubcategoria}" requiere subcategoría anidada`);
        await showStepMessage(page, `🎯 SELECCIONANDO SUBCATEGORÍA ANIDADA (${servicioNum}/${totalServicios})`);
        await page.waitForTimeout(2000);

        try {
          nombreSubcategoriaAnidada = await selectRandomCategory(page, `SELECCIONANDO SUBCATEGORÍA ANIDADA (${servicioNum}/${totalServicios})`);
          await page.waitForTimeout(2000);
          console.log(`✅ Subcategoría anidada "${nombreSubcategoriaAnidada}" seleccionada exitosamente`);
        } catch (error) {
          console.log(`⚠️ No se pudo seleccionar subcategoría anidada: ${error}`);
      ...
      // --- LLENAR FORMULARIO DE DATOS DEL SERVICIO ---
      await showStepMessage(page, `📝 LLENANDO DATOS DEL SERVICIO (${servicioNum}/${totalServicios})`);
      await page.waitForTimeout(1000);

      // Función para generar nombres apropiados según la categoría
      function generateServiceName(category: string, subcategory: string): string {
        const categoryNames: { [key: string]: string[] } = {
          'Alimentos': ['Catering Gourmet', 'Banquete Especial', 'Cocina Tradicional', 'Servicio Gastronómico', 'Menú Ejecutivo', 'Buffet Premium', 'Comida Artesanal', 'Culinaria Exclusiva'],
          'Bebidas': ['Bar Premium', 'Coctelería Artesanal', 'Servicio de Bebidas', 'Barra Libre', 'Bebidas Especiales', 'Cocteles Premium', 'Servicio de Licores', 'Barra Personalizada'],
          'Lugares': ['Salón de Eventos', 'Espacio Versátil', 'Lugar Exclusivo', 'Venue Premium', 'Espacio Elegante', 'Salón VIP', 'Lugar Único', 'Espacio Especial'],
          'Mobiliario': ['Mobiliario Premium', 'Equipamiento Completo', 'Mobiliario Elegante', 'Sillas y Mesas', 'Mobiliario Versátil', 'Equipamiento VIP', 'Mobiliario Personalizado', 'Sillas Especiales'],
          'Entretenimiento': ['Show en Vivo', 'Entretenimiento Premium', 'Actuación Especial', 'Show Personalizado', 'Entretenimiento VIP', 'Actuación Única', 'Show Exclusivo', 'Entretenimiento Artesanal'],
          'Música': ['Grupo Musical', 'DJ Premium', 'Música en Vivo', 'Sonido Profesional', 'Música Personalizada', 'DJ Especializado', 'Grupo Exclusivo', 'Música Artesanal'],
          'Decoración': ['Decoración Temática', 'Ambientación Premium', 'Decoración Personalizada', 'Diseño Exclusivo', 'Ambientación Elegante', 'Decoración Única', 'Diseño Especial', 'Ambientación Artesanal'],
          'Invitaciones': ['Invitaciones Elegantes', 'Diseño Personalizado', 'Tarjetas Premium', 'Invitaciones Únicas', 'Diseño Exclusivo', 'Tarjetas Especiales', 'Invitaciones Artesanales', 'Diseño Versátil'],
          'Mesa de regalos': ['Mesa de Regalos', 'Lista de Regalos', 'Registros Especiales', 'Mesa Personalizada', 'Lista Premium', 'Registros Únicos', 'Mesa Exclusiva', 'Lista Artesanal'],
          'Servicios Especializados': ['Servicio Especializado', 'Servicio Premium', 'Servicio Personalizado', 'Servicio Exclusivo', 'Servicio Único', 'Servicio Artesanal', 'Servicio Versátil', 'Servicio Elegante']
        };
        ...
        return `${finalName} - ${servicioNum}`;
      }

      // Generar datos dinámicos para el servicio
      const now = new Date();
      const serviceName = generateServiceName(tituloCategoria || 'Servicios Especializados', subcategoriaFinal || 'General');
      const serviceDescription = `Descripción del servicio de ${subcategoriaFinal} creado el ${now.toLocaleDateString()} - Servicio ${servicioNum}`;
      const minCapacity = Math.floor(Math.random() * 10) + 1;
      const maxCapacity = minCapacity + Math.floor(Math.random() * 50) + 10;
      ...
      // Seleccionar unidades aleatorias
      const units = page.locator('#Units button[type="button"]');
      const unitCount = await units.count();
      const selectedUnits = Math.floor(Math.random() * 3) + 1;

      for (let j = 0; j < selectedUnits; j++) {
        const randomUnitIndex = Math.floor(Math.random() * unitCount);
        await units.nth(randomUnitIndex).click();
        await page.waitForTimeout(500);
      }
      ...
      // Enviar formulario
      console.log(' Enviando formulario de detalles...');
      await showStepMessage(page, `➡️ ENVIANDO FORMULARIO (${servicioNum}/${totalServicios})`);
      await page.waitForTimeout(1000);
      ...
      // --- LLENAR FORMULARIO DE PRECIOS Y CONDICIONES ---
      await showStepMessage(page, `💰 CONFIGURANDO PRECIOS Y CONDICIONES (${servicioNum}/${totalServicios})`);
      await page.waitForTimeout(2000);
      ...
      // Seleccionar unidad del dropdown
      await showStepMessage(page, `📏 SELECCIONANDO UNIDAD (${servicioNum}/${totalServicios})`);
      await page.waitForTimeout(1000);
      ...
      const paymentMethods = page.locator('#PaymentMethod button[type="button"]');
      const paymentCount = await paymentMethods.count();
      const selectedPayments = Math.floor(Math.random() * 2) + 1;

      for (let j = 0; j < selectedPayments; j++) {
        const randomPaymentIndex = Math.floor(Math.random() * paymentCount);
        await paymentMethods.nth(randomPaymentIndex).click();
        await page.waitForTimeout(500);
      }
      ...
      // Opcional: marcar "Requiere anticipo"
      if (Math.random() > 0.5) {
        await page.locator('label[for="Advance"]').click();
        await page.waitForTimeout(500);
      }
      ...
      // Enviar formulario de precios
      console.log(' Enviando formulario de precios...');
      await showStepMessage(page, `➡️ ENVIANDO FORMULARIO DE PRECIOS (${servicioNum}/${totalServicios})`);
      await page.waitForTimeout(1000);
      ...
      // --- SELECCIONAR ATRIBUTOS ALEATORIOS ---
      await showStepMessage(page, `🎯 SELECCIONANDO ATRIBUTOS DEL SERVICIO (${servicioNum}/${totalServicios})`);
      await page.waitForTimeout(2000);
      ...
      if (attributeCount > 0) {
        const selectedAttributes = Math.floor(Math.random() * 3) + 1;

        for (let j = 0; j < selectedAttributes && j < attributeCount; j++) {
          const randomAttributeIndex = Math.floor(Math.random() * attributeCount);
          const checkbox = attributeCheckboxes.nth(randomAttributeIndex);
          const checkboxId = await checkbox.getAttribute('id');
          if (checkboxId) {
      ...
      // Enviar formulario de atributos
      console.log(' Enviando formulario de atributos...');
      await showStepMessage(page, `➡️ ENVIANDO FORMULARIO DE ATRIBUTOS (${servicioNum}/${totalServicios})`);
      await page.waitForTimeout(1000);
      ...
      if (necesitaRango) {
        await showStepMessage(page, `📍 CONFIGURANDO RANGO DE SERVICIO (${servicioNum}/${totalServicios})`);
        await page.waitForTimeout(2000);
        ...
        await showStepMessage(page, `➡️ ENVIANDO FORMULARIO DE RANGO (${servicioNum}/${totalServicios})`);
        await page.waitForTimeout(1000);
        ...
        console.log('📍 Categoría "Lugares" detectada - omitiendo configuración de rango de servicio');
        await showStepMessage(page, `📍 OMITIENDO RANGO DE SERVICIO (${servicioNum}/${totalServicios})`);
        await page.waitForTimeout(2000);
      }
      ...
      try {
        ...
        await showStepMessage(page, `📸 SUBIENDO IMAGEN DE PRUEBA (${servicioNum}/${totalServicios})`);
        await page.waitForTimeout(1000);
        const fileInput = page.locator('input[type="file"]');
        ...
        try {
          const button1 = page.locator('button[type="submit"][form="ServiceMediaForm"]');
          await expect(button1).toBeVisible({ timeout: 30000 });
          finalSubmitButton = button1;
          buttonFound = true;
        } catch (error) {
        ...
        // Página de confirmación con botón "Finalizar"
        await showStepMessage(page, `✅ CONFIRMACIÓN: CLIC EN "FINALIZAR" (${servicioNum}/${totalServicios})`);
        ...
        try {
          ...
          console.log(`⚠️ Error al buscar botón alternativo: ${altError}`);
        }
      }

      // Esperar regreso al administrador de servicios
      console.log(' Esperando regreso al administrador de servicios...');
      let regresoExitoso = false;
      ...
      if (!regresoExitoso) {
        const currentUrl = page.url();
        throw new Error(`❌ No se pudo regresar al administrador de servicios después de crear el servicio. URL actual: ${currentUrl}`);
      }

      serviciosCreados++;
      console.log(`✅ Servicio ${servicioNum}/${totalServicios} "${serviceName}" creado exitosamente`);
      ...
      serviciosFallidos++;
      console.error(`❌ Error al crear servicio ${servicioNum}/${totalServicios}: ${error.message}`);
      
      // Intentar regresar a la página de servicios
      try {
        await page.goto(PROVIDER_SERVICES_URL);
        await page.waitForTimeout(2000);
      } catch (e) {
  ...
  // --- RESUMEN FINAL ---
  console.log(`\n📊 RESUMEN DE CREACIÓN:`);
  console.log(`   ✅ Servicios creados exitosamente: ${serviciosCreados}/${totalServicios}`);
  console.log(`   ❌ Servicios fallidos: ${serviciosFallidos}/${totalServicios}`);
  
  // Validar que se crearon todos los servicios
  expect(serviciosCreados).toBeGreaterThan(0);
  console.log(`\n✅ Prueba completada: Se crearon ${serviciosCreados} servicio(s) de ${totalServicios}`);
});
